...
function changePos() {
  ...
  if (scrollTop > catalogTopHeight - 20) {
    catalog.style = 'position: fixed; top: 20px; bottom: 20px; right: 25px';
  } else {
...
function isActiveCat() {
  ...
  for (let i = 0; i < catLinkList.length; i++) {
    ...
    let nextTopCat = i + 1 === headerLinkList.length ? Infinity : headerLinkList[i + 1].offsetTop - offsetHeight;
